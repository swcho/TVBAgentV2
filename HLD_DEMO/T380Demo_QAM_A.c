...
{
	int		Modulator_Type = 2;//QAM-Annex.A
	long    Playrate;
	int     Board_Id=-1;
	int     Repeat;
	int		Constellation;
	...
	if ( argc > 1 ) 
	{
		...
		{
		#ifdef WIN32
			printf("Usage:hld_mod_qam_a.exe");
		#else
			printf("Usage:mod_qam_a");
		#endif
			...
			printf("\nrepeat_mode");
			printf("\nplayrate");
			printf("\nconstellation(16QAM:0, 32QAM:1, 64QAM:2, 128QAM:3, 256QAM:4)");
			printf("\nsymbol rate(1000000~8000000, Symbol/sec)");
			printf("\nRF(Hz)");
			printf("\nIF(36000000 or 44000000 Hz)");
			printf("\nattenuation(0~31.5, +/-0.1 dB, default:18dB)"); 
...
//	#endif
		...
		/* Modulator Paramters */
		printf("Constellation(16QAM:0, 32QAM:1, 64QAM:2, 128QAM:3, 256QAM:4) : ");
		fgets(line, sizeof(line), stdin);
		...
		printf("Symbol rate(Sps)(1000000~8000000) : ");
		fgets(line, sizeof(line), stdin);
		...
		if ( Symbol_Rate < 1000000 || Symbol_Rate > 8000000 )
		{
		...
		printf("Attenuation(dB)(0.0~31.5, +/-0.1) : (default:18.0) ");
		fgets(line, sizeof(line), stdin);
		sscanf(line, "%f", &Atten);
		dwBufferSize = strlen(line)-1;
		line[dwBufferSize] = '\0';
	...
	Max_Playrate = (long)((Symbol_Rate / (204. / 188.)) * (Constellation + 4.));
...
//	}
	...
	{
		if ( nBypassAmp == 0 )
			printf("Output level option is 0dBm. The default atten(21dBm) may be required.\n");
		else
			printf("The default atten(14dBm) may be required.\n");
	}
	else
	{
		nOptionalOutputAmp = TSPL_GET_BOARD_CONFIG_STATUS_EX(Board_Id);
		if ( nOptionalOutputAmp == 1 )
			printf("Output level option is -6dBm. The default atten(25dBm) may be required.\n");
		else if ( nOptionalOutputAmp == 2 )
			printf("Output level option is 0dBm. The default atten(25dBm) may be required.\n");
	}

...
int	main(int argc, char *argv[])
{
	...
	if ( argc > 1 ) 
	{
		...
		{
		#ifdef WIN32
			printf("Usage:hld_mod_qpsk.exe");
		#else
			printf("Usage:mod_qpsk");
		#endif
...
//	#endif
		...
		//RRC filter
		printf("RRC filter(off : 0, on : 1) : ");
		fgets(line, sizeof(line), stdin);
		sscanf(line, "%d", &nRRC_filter);
		dwBufferSize = strlen(line)-1;
		line[dwBufferSize] = '\0';
		if ( strlen(line) == 0 )
		{
		...
		if ( nRRC_filter < 0 || nRRC_filter > 1 )
		{
			printf("Invalid RRC filter.\n");
			return 0;
		}
	...
	Max_Playrate = (long)((Symbol_Rate * 2. / (204. / 188.)) * dwCoderate);
...
//	}
		...
		|| nBoardTypeID == 0x16 ) //TVB591S 
	{
		...
		//RRC filter
		TVB380_SET_MODULATOR_ROLL_OFF_FACTOR_EX(Board_Id, Modulator_Type, nRRC_filter);
	}

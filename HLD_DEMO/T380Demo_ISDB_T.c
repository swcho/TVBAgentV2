...
{
	int		Modulator_Type = 9;//ISDB-T 1 SEGMENT, 10=ISDB-T 13 SEGMENT
	long    Playrate = 32507936;
	int     Board_Id=-1;
	int     Repeat;
	long	RF_Freq, IF_Freq;
	long	Symbol_Rate = 8126984;
	...
	if ( argc > 1 ) 
	{
		if ( argc < cnt )
		{
			...
			printf("\nmodulator type(1 seg. : 0, 13 seg. : 1)");
...
//	#endif
		...
		/* Modulator Paramters */
		printf("RF freq.(Hz),###.143000MHz : ");//142857 -> 143000
		fgets(line, sizeof(line), stdin);
		...
		printf("Modulator type(1 seg. : 0, 13 seg. : 1) : ");
		fgets(line, sizeof(line), stdin);
		sscanf(line, "%d", &Modulator_Type);
		dwBufferSize = strlen(line)-1;
		line[dwBufferSize] = '\0';
		if ( strlen(line) == 0 )
		{
		...
		if ( Modulator_Type != 0 && Modulator_Type != 1 )
		{
			printf("Invalid parameter.\n");
			return 0;
		}
...
//	}
	...
	if ( nBoardTypeID == 0x30 
		...
		|| nBoardTypeID == 0x16 ) //TVB591S 
	{
		if ( Modulator_Type == 10 ) //ISDB-T 13 SEGMENT
		{

...
{
	int		Modulator_Type = 12;//CMMB
	long    Playrate;
	int     Board_Id=-1;
	int     Repeat;
	long	RF_Freq, IF_Freq;
	long 	Symbol_Rate = 20000000;
...
//	#endif
		...
		printf("File name to play(.mfs, .mmx) : ");
...
//	}
	...
	do 
	{
		...
		//Play
		TSPH_START_PLAY(Board_Id, fileName, Playrate, 0, 1/* always 1*/, Repeat);
		if ( Repeat < 1 )
		{

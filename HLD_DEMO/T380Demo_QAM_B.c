...
{
	int		Modulator_Type = 3;//QAM-Annex.B
	long    Playrate;
	int     Board_Id=-1;
	int     Repeat;
	int		Constellation;
	...
	if ( argc > 1 ) 
	{
		...
		{
		#ifdef WIN32
			printf("Usage:hld_mod_qam_b.exe");
		#else
			printf("Usage:mod_qam_b");
		#endif
			...
			printf("\nrepeat_mode");
			printf("\nplayrate");
			printf("\nconstellation(64QAM:0, 256QAM:1)");
			printf("\ninterleave(128-1:0, 64-2:1,..., 128-7:11, 128-8:12)");
			printf("\nRF(Hz)");
			printf("\nIF(36000000 or 44000000 Hz)");
			printf("\nattenuation(0~31.5, +/-0.1 dB, default:26dB)"); 
...
//	#endif
		...
		/* Modulator Paramters */
		printf("Constellation(64QAM:0, 256QAM:1) : ");
		fgets(line, sizeof(line), stdin);
		...
		printf("Symbol rate(Sps)(64QAM:5056941, 256QAM:5360537) : ");
		fgets(line, sizeof(line), stdin);
		...
		if ( (Constellation == 0 && Symbol_Rate != 5056941) 
			|| (Constellation == 1 &&  Symbol_Rate != 5360537) )
		{
			printf("Invalid symbol rate.\n");
			return 0;
		}

		printf("Interleave(128-1:0, 64-2:1,..., 128-7:11, 128-8:12) : ");
		fgets(line, sizeof(line), stdin);
		...
		printf("Attenuation(dB)(0.0~31.5, +/-0.1) : (default:26.0) ");
		fgets(line, sizeof(line), stdin);
		sscanf(line, "%f", &Atten);
		dwBufferSize = strlen(line)-1;
		line[dwBufferSize] = '\0';
	...
	if ( Constellation == 0 )
		Max_Playrate = (long)(Symbol_Rate * 6. * (122. / 128.) * (53760. / 53802.) * (14. / 15.));
	else
		Max_Playrate = (long)(Symbol_Rate * 8. * (122. / 128.) * (78848. / 78888.) * (19. / 20.));
...
//	}
	...
	{
		if ( nBypassAmp == 0 )
			printf("Output level option is 0dBm. The default atten(24dBm) may be required.\n");
		else
			printf("The default atten(22dBm) may be required.\n");
	}
	else
	{
		nOptionalOutputAmp = TSPL_GET_BOARD_CONFIG_STATUS_EX(Board_Id);
		if ( nOptionalOutputAmp == 1 )
			printf("Output level option is -6dBm. The default atten(28dBm) may be required.\n");
		else if ( nOptionalOutputAmp == 2 )
			printf("Output level option is 0dBm. The default atten(28dBm) may be required.\n");
	}
	...
	TVB380_SET_MODULATOR_INTERLEAVE_EX(Board_Id, Modulator_Type, Interleave);

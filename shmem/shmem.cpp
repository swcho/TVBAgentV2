...
//    return semp_init(&e, (char*)name, signaled) == 0;
	...
	IsOk = (semp_init(&e, (char*)name, signaled) == 0);
	if (!IsOk)
	{
		IsOk = (semp_open(&e, (char*)name) == 0);//???
	}
    	return IsOk;
}
...
void* shared_memory::allocate(size_t size, bool initialize_by_zero)
{
    ...
    while (true) { 
    	...
    	bp = bp->next;
    	if (bp == curr) { // hole not foundprintf("VLC_Recv_Stream....11111\n");			
			if (pHdr->size + size + sizeof(allocation_block) > pMonitor->max_file_size)
			{

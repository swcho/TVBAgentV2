...
//ref:  CRC-8 table
//polynomial: /* x^8 + x^7 + x^6 + x^4 + x^2 + 1 */, GP  0x1D5, DI  0xD5
//reference: Annex F, ETSI EN 302 755
...
// crc-32 table
// polynomial: x^32 + x^26 + x^23 + x^21 + x^16 + x^12 + x^11 + x^10 + x^8 + x^7 + x^5 + x^4 + x^2 + x + 1
// refer: Annex ETSI TS 102 773
...
unsigned char crc8(unsigned char *buff, int len, unsigned char init_value)
{
    unsigned char crc = init_value;
    int i;
    for (i = 0; i < len; i++)
    {
        crc = crc8_table[buff[i] ^ crc];
    }
    return crc;
}

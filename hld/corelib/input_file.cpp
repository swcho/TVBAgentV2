	...
	while(thread_handle->thread_state == THREAD_STATE_READING_INPUT)
	{
		...
		if (buff_index_last > 0)
		{
			if(sizeof(long) > 4)
			{
				...
				if(temp_buf != NULL)
				{
					memcpy(&temp_buf[0], &buff[buff_index_start], buff_index_last - buff_index_start);
					memcpy(&buff[0], &temp_buf[0], buff_index_last - buff_index_start);
					free(temp_buf);
				}
				else
...
CTLVFileDVBT2Mul::CTLVFileDVBT2Mul()
{
	...
	//debug_fp = fopen("c:\\testfile.ts", "wb");
	debug_fp = NULL;
}
...
{
	static char tmp_buf[FILE_READ_DATA_SIZE_MAX*2];
	if ((sys_play->play_mode & PLAY_MODE_REPEAT) == PLAY_MODE_REPEAT)
	if (feof(file_handler) != 0)
	{
		fseek(file_handler, 0, SEEK_SET);
	...
	if (ts_buffer_start > 0)
	{
		if(sizeof(long) > 4)
		{
			...
			if(ts_buffer_start + ts_buffer_size > FILE_READ_DATA_SIZE_MAX*2)
				printf("==================\n");
			memcpy(&tmp_buf[0], &ts_buffer[ts_buffer_start], ts_buffer_size);
			memcpy(&ts_buffer[0], &tmp_buf[0], ts_buffer_size);
#endif
		}
		else
			memcpy(&ts_buffer[0], &ts_buffer[ts_buffer_start], ts_buffer_size);

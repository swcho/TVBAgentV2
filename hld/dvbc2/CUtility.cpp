...
unsigned int HCRC32::crc_table[256] = {
...
unsigned int HCRC32::crc32 (const char *d, int len, unsigned int crc)
{
   register int i;
...
HCRC32::HCRC32(const char *d, int len, unsigned int CRCvalue) {
   data=d;
   length=len;
   value=CRCvalue;
}
...
//HCRC8
unsigned char HCRC8::crc8_table[256] = {
...
*/
void HCRC8::crc8(unsigned char *crc, unsigned char m)
{
//	if (!made_table)
//		init_crc8();
...
//CRC
int CUtilities::MakeCRC(unsigned char *Target, const unsigned char *Data, int Length)
{
  unsigned int crc = HCRC32::crc32((const char *)Data, Length, 0xFFFFFFFF);
  int i = 0;
  Target[i++] = crc >> 24;
  Target[i++] = crc >> 16;
  Target[i++] = crc >> 8;
  Target[i++] = crc;
  //LPRINT(fn_Log,"new...CRC=0x%x, 0x%x, 0x%x, 0x%x\n\n", Target[0], Target[1], Target[2], Target[3]);
  return i;
}

// shift left syncd bit from second bytes
void CUtilities::BitConcat(unsigned char *des, int syncd, int num_bits){

	
	int length_in_bytes = (num_bits - 1)/8 + 1;	
	unsigned char *shift;
	unsigned char first_byte = des[0];
	des[0] = 0x00;
...
int CUtilities::AddBitToBuffer(unsigned char *des, int des_length_in_bit, unsigned char *src, int src_length_in_bit){	

#if 0

	int des_index_in_byte= (des_length_in_bit)/8 ;
	int des_length_remainer = des_length_in_bit%8;

	int src_length_in_byte = (src_length_in_bit -1 )/8 + 1;	

	int syncd;
	unsigned char byte_align_des = *(des + des_index_in_byte);
	unsigned char byte_align_src = *(src);

	memcpy(des + des_index_in_byte
	...
	if (src_length_in_bit > 0)
	{
		...
		memcpy(&des[des_index_in_byte], &src[0], src_length_in_byte); 
		...
		CUtilities::BitConcat(&des[des_index_in_byte - 1], syncd, src_length_in_bit);
	}
#endif	
...
int CUtilities::TL_Calculate_L1_Post_Size(int L1_MOD, int FFT_SIZE, int L1_POST_INFO_SIZE)
{
